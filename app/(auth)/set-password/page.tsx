  ...
  if (error && !userEmail) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="max-w-md w-full rounded-2xl bg-foreground/5 border border-border p-8 text-center">
          <AlertCircle className="h-12 w-12 text-red-400 mx-auto mb-4" />
          <h1 className="text-xl font-bold text-foreground mb-2">Invalid Link</h1>
          <p className="text-muted-foreground mb-6">{error}</p>
          <Link href="/forgot-password" prefetch={false} className="block w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold">
            Request New Link
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-4">
        ...
        <div className="rounded-2xl bg-foreground/5 border border-border p-8">
          <h1 className="text-2xl font-bold text-foreground mb-2 text-center">Set Password</h1>
          {userEmail && <p className="text-muted-foreground text-center mb-6">{userEmail}</p>}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required
                  minLength={8}
                  className="w-full py-3 pl-10 pr-10 rounded-xl bg-foreground/5 border border-border text-foreground"
                  placeholder="••••••••"
                />
                <button
                  type="button"
            ...
            <div>
              <label className="block text-sm text-foreground/70 mb-1">Confirm Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                <input
