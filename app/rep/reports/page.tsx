      ...
      <div className="px-6 py-12 md:px-12 lg:px-24 md:py-16">
        <div className="mx-auto max-w-6xl space-y-12">
          <div className="space-y-4">
            <h1 className="text-5xl font-bold tracking-tighter text-foreground md:text-6xl">Reports</h1>
      ...
      <div className="mx-auto max-w-6xl space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            <h1 className="text-5xl font-bold tracking-tighter text-foreground md:text-6xl">Reports</h1>
            <p className="text-xl text-muted-foreground">Generate invoices and custom reports</p>
          </div>
          <div className="flex items-center gap-3">
            <Button
              onClick={() => setShowFilters(!showFilters)}
              variant="outline"
              className="h-11 px-5 bg-foreground/5 hover:bg-foreground/10 text-foreground border-border hover:border-border rounded-xl"
            >
            ...
            <Button
              onClick={exportToCSV}
              className="h-11 px-5 bg-primary text-primary-foreground hover:bg-card/90 rounded-xl font-semibold"
            >
        ...
        <section className="space-y-4">
          <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Report Type</h2>
              ...
              return (
                <button
                  ...
                  className={`relative p-5 rounded-2xl border text-left transition-all ${
                    isActive
                      ? "bg-foreground/10 border-border shadow-lg"
                      : "bg-foreground/5 border-border hover:bg-foreground/[0.08] hover:border-primary/15"
                  }`}
                >
                  <div className="flex items-start gap-4">
                    <div className={`p-3 rounded-xl ${isActive ? "bg-emerald-500/20" : "bg-foreground/10"}`}>
                      <Icon className={`w-5 h-5 ${isActive ? "text-emerald-400" : "text-foreground/60"}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className={`font-semibold ${isActive ? "text-foreground" : "text-foreground/80"}`}>{type.label}</h3>
        ...
        <AnimatePresence>
          {showFilters && (
            ...
            >
              <div 
                className="p-6 rounded-2xl border border-border backdrop-blur-xl space-y-6"
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      className="w-full h-11 px-4 rounded-xl bg-foreground/5 border border-border text-foreground focus:border-border focus:outline-none"
                    >
                      {DATE_RANGES.map(range => (
                        <option key={range.id} value={range.id} className="bg-background">{range.label}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  {/* Customer Search */}
                  <div className="space-y-2">
                    <label className="text-sm text-foreground/60">Customer</label>
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className="h-11 pl-10 bg-foreground/5 border-border text-foreground placeholder:text-muted-foreground rounded-xl"
                      />
                    </div>
                  </div>
                  ...
                  {/* Amount Range */}
                  <div className="space-y-2">
                    <label className="text-sm text-foreground/60">Amount Range</label>
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        placeholder="Min"
                        className="h-11 bg-foreground/5 border-border text-foreground placeholder:text-muted-foreground rounded-xl"
                      />
                ...
                {filters.dateRange === "custom" && (
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border">
                    <div className="space-y-2">
                      <label className="text-sm text-foreground/60">Start Date</label>
                      <Input
                        type="date"
                        value={filters.customStartDate || ""}
                        onChange={(e) => setFilters({ ...filters, customStartDate: e.target.value })}
                        className="h-11 bg-foreground/5 border-border text-foreground rounded-xl"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm text-foreground/60">End Date</label>
                      <Input
                        type="date"
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div 
            className="p-5 rounded-2xl border border-border backdrop-blur-xl"
            style={{ backgroundColor: 'rgba(58,66,51,0.15)' }}
          >
        ...
        <section className="space-y-6">
          ...
          <div 
            className="rounded-2xl border border-border backdrop-blur-xl overflow-hidden"
            style={{ backgroundColor: 'rgba(58,66,51,0.15)' }}
          >
            {/* Invoice Report */}
                    ...
                    filteredOrders.map(order => (
                      ...
                      >
                        <div className="col-span-1">
                          ...
                          >
                            {selectedOrders.has(order.id) && <Check className="w-3 h-3 text-foreground" />}
                          </button>
                        </div>
                        <div className="col-span-2">
                          <span className="font-mono text-sm text-foreground/70">{order.id.slice(0, 8)}...</span>
                        </div>
            ...
            {selectedReport === "sales" && (
              <div className="p-6 space-y-6">
                ...
                <div className="border-t border-border pt-6">
                  <h3 className="text-lg font-semibold text-foreground mb-4">Order Breakdown</h3>
                  <div className="space-y-3">
                    {filteredOrders.slice(0, 10).map(order => (
                      <div key={order.id} className="flex items-center justify-between p-3 rounded-lg bg-foreground/5">
                        ...
                        <div className="flex items-center gap-6">
                          <Badge className="bg-foreground/10 text-foreground/70 border-0">{order.status}</Badge>
                  ...
                  Object.values(customerGroups).map(group => (
                    <div key={group.customer.id} className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="text-lg font-semibold text-foreground">{group.customer.name}</h3>
                          <p className="text-sm text-muted-foreground">{group.customer.email}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-2xl font-bold text-emerald-400">${group.totalSpent.toFixed(2)}</p>
                          <p className="text-sm text-muted-foreground">{group.orders.length} orders</p>
                        </div>
                      </div>
                      <div className="space-y-2">
                        {group.orders.slice(0, 3).map(order => (
                          <div key={order.id} className="flex items-center justify-between text-sm p-2 rounded-lg bg-foreground/5">
                            <span className="text-foreground/60">{format(new Date(order.date), "MMM dd, yyyy")}</span>
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="p-6 rounded-xl bg-foreground/5 border border-border">
                    ...
                    <p className="text-sm text-muted-foreground mt-2">Of order total</p>
                  </div>
                </div>

                <div className="border-t border-border pt-6">
                  <h3 className="text-lg font-semibold text-foreground mb-4">Commission by Order</h3>
                  <div className="space-y-2">
                    {filteredOrders.slice(0, 15).map(order => (
                      <div key={order.id} className="flex items-center justify-between p-3 rounded-lg bg-foreground/5">
                        <div className="flex items-center gap-4">
                          <span className="font-mono text-sm text-muted-foreground">{order.id.slice(0, 8)}</span>
                          <span className="text-foreground">{order.customer.name}</span>
                          <span className="text-muted-foreground text-sm">{format(new Date(order.date), "MMM dd")}</span>
                        </div>
                        <div className="flex items-center gap-6">
                          <span className="text-foreground/60">${order.total.toFixed(2)}</span>
            ...
            >
              ...
              <div className="sticky top-0 flex items-center justify-between p-6 border-b border-border bg-card">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => setPreviewOrder(null)}
                    className="p-2 rounded-lg hover:bg-foreground/10 text-foreground/60 hover:text-foreground"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
              ...
              <div className="p-8 space-y-8">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-muted-foreground mt-1">#{previewOrder.id.slice(0, 8).toUpperCase()}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-foreground/60">Date</p>
                    <p className="text-foreground font-medium">{format(new Date(previewOrder.date), "MMMM dd, yyyy")}</p>
                  </div>
                </div>

                {/* Bill To */}
                <div className="p-4 rounded-xl bg-foreground/5 border border-border">
                  <p className="text-sm text-muted-foreground uppercase tracking-wider mb-2">Bill To</p>
                  <p className="text-lg font-semibold text-foreground">{previewOrder.customer.name}</p>
                  <p className="text-foreground/60">{previewOrder.customer.email}</p>
                </div>

                {/* Items */}
                <div>
                  <p className="text-sm text-muted-foreground uppercase tracking-wider mb-4">Items</p>
                  <div className="space-y-3">
                    {previewOrder.items.length === 0 ? (
                      <p className="text-muted-foreground text-center py-4">No items recorded</p>
                    ) : (
                      previewOrder.items.map(item => (
                        <div key={item.id} className="flex justify-between items-center p-4 rounded-xl bg-foreground/5">
                          <div>
                ...
                <div className="border-t border-border pt-6">
                  <div className="flex justify-between items-center">
                    <span className="text-xl text-foreground/60">Total</span>
                    <span className="text-3xl font-bold text-foreground">${previewOrder.total.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div className="text-center pt-6 border-t border-border">
                  ...
                  <p className="text-sm text-muted-foreground mt-2">Generated by {repName}</p>
                </div>
              </div>
            </motion.div>
          </motion.div>
