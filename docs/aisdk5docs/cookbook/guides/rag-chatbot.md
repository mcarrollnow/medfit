...
[Embeddings](../../docs/ai-sdk-core/embeddings.html) are a way to represent words, phrases, or images as vectors in a high-dimensional space. In this space, similar words are close to each other, and the distance between words can be used to measure their similarity.
...
If you experience an error with the migration, see the [troubleshooting section](#troubleshooting-migration-error) below.
...
``` tsx
import  from '@/lib/utils';import  from 'drizzle-orm/pg-core';import  from './resources';
export const embeddings = pgTable(  'embeddings',  )      .primaryKey()      .$defaultFn(() => nanoid()),    resourceId: varchar('resource_id', ).references(      () => resources.id,      ,    ),    content: text('content').notNull(),    embedding: vector('embedding', ).notNull(),  },  table => (),);
```
...
This will install the [AI SDK](../../docs/introduction.html), AI SDK's React hooks, and AI SDK's [OpenAI provider](../../providers/ai-sdk-providers/openai.html).
...
const generateChunks = (input: string): string[] => ;
export const generateEmbeddings = async (  value: string,): Promise<Array<>> =>  = await embedMany();  return embeddings.map((e, i) => ());};
```
...
``` tsx
'use server';
import  from '@/lib/db/schema/resources';import  from '../db';
export const createResource = async (input: NewResourceParams) =>  = insertResourceSchema.parse(input);
    const [resource] = await db      .insert(resources)      .values()      .returning();
    return 'Resource successfully created.';  } catch (e) };
```
...
``` tsx
'use server';
import  from '@/lib/db/schema/resources';import  from '../db';import  from '../ai/embedding';import  from '../db/schema/embeddings';
export const createResource = async (input: NewResourceParams) =>  = insertResourceSchema.parse(input);
    const [resource] = await db      .insert(resources)      .values()      .returning();
    const embeddings = await generateEmbeddings(content);    await db.insert(embeddingsTable).values(      embeddings.map(embedding => ()),    );
    return 'Resource successfully created and embedded.';  } catch (error) };
```
...
Great! Let's build the frontend. The AI SDK’s [`useChat`](../../docs/reference/ai-sdk-ui/use-chat.html) hook allows you to easily create a conversational user interface for your agent.
...
export default function Chat()  = useChat();  return (    <div className="flex flex-col w-full max-w-md py-24 mx-auto stretch">      <div className="space-y-4">         className="whitespace-pre-wrap">            <div>              <div className="font-bold"></div>              </p>;                }              })}            </div>          </div>        ))}      </div>
...
In this code, you declare and export an asynchronous function called POST. You retrieve the `messages` from the request body and then pass them to the [`streamText`](../../docs/reference/ai-sdk-core/stream-text.html) function imported from the AI SDK, alongside the model you would like to use. Finally, you return the model’s response in `UIMessageStreamResponse` format.
...
A [tool](../../docs/foundations/tools.html) is a function that can be called by the model to perform a specific task. You can think of a tool like a program you give to the model that it can run as and when it deems necessary.
...
``` tsx
import  from '@/lib/actions/resources';import  from '@ai-sdk/openai';import  from 'ai';import  from 'zod';
// Allow streaming responses up to 30 secondsexport const maxDuration = 30;
export async function POST(req: Request) :  = await req.json();
  const result = streamText(),        execute: async () => createResource(),      }),    },  });
  return result.toUIMessageStreamResponse();}
```
...
- **inputSchema**: [Zod schema](../../docs/foundations/tools.html#schema-specification-and-validation-with-zod) that defines the input necessary for the tool to run.
...
'use client';
import  from '@ai-sdk/react';import  from 'react';
export default function Chat()  = useChat();  return (    <div className="flex flex-col w-full max-w-md py-24 mx-auto stretch">      <div className="space-y-4">         className="whitespace-pre-wrap">            <div>              <div className="font-bold"></div>              </p>;                  case 'tool-addResource':                  case 'tool-getInformation':                    return (                      <p>                        call                        tool:                         <pre className="my-4 bg-zinc-100 p-2 rounded-sm">                                                  </pre>                      </p>                    );                }              })}            </div>          </div>        ))}      </div>
      <form        onSubmit=);          setInput('');        }}      >        <input          className="fixed bottom-0 w-full max-w-md p-2 mb-8 border border-gray-300 rounded shadow-xl"          value=          placeholder="Say something..."          onChange=        />      </form>    </div>  );}
```
...
The AI SDK has a feature called [`stopWhen`](../../docs/ai-sdk-core/tools-and-tool-calling.html#multi-step-calls) which allows stopping conditions when the model generates a tool call. If those stopping conditions haven't been hit, the AI SDK will automatically send tool call results back to the model!
...
``` tsx
import  from 'ai';import  from '@ai-sdk/openai';import  from '../db';import  from 'drizzle-orm';import  from '../db/schema/embeddings';
const embeddingModel = openai.embedding('text-embedding-ada-002');
const generateChunks = (input: string): string[] => ;
export const generateEmbeddings = async (  value: string,): Promise<Array<>> =>  = await embedMany();  return embeddings.map((e, i) => ());};
export const generateEmbedding = async (value: string): Promise<number[]> =>  = await embed();  return embedding;};
export const findRelevantContent = async (userQuery: string) => )`;  const similarGuides = await db    .select()    .from(embeddings)    .where(gt(similarity, 0.5))    .orderBy(t => desc(t.similarity))    .limit(4);  return similarGuides;};
```
...
``` ts
import  from '@/lib/actions/resources';import  from '@ai-sdk/openai';import  from 'ai';import  from 'zod';import  from '@/lib/ai/embedding';
// Allow streaming responses up to 30 secondsexport const maxDuration = 30;
export async function POST(req: Request) :  = await req.json();
  const result = streamText(),        execute: async () => createResource(),      }),      getInformation: tool(),        execute: async () => findRelevantContent(question),      }),    },  });
  return result.toUIMessageStreamResponse();}
```
