...
``` ts
/* ...other imports... */import  from 'ai';import  from '@ai-sdk/openai';import  from 'zod';
/* ...rest of the file... */
export const generateQuery = async (input: string) => `,      schema: z.object(),    });    return result.object.query;  } catch (e) };
```
...
``` typescript
/* ...other imports... */import  from './actions';
/* ...rest of the file... */
...
When you explain you must take a section of the query, and then explain it. Each "section" should be unique. So in a query like: "SELECT * FROM unicorns limit 20", the sections could be "SELECT *", "FROM UNICORNS", "LIMIT 20".If a section doesn't have any explanation, include it, but leave the explanation empty.
```
...
export const explainQuery = async (input: string, sqlQuery: string) => 
      Generated SQL Query:      $`,    });    return result.object;  } catch (e) };
```
...
``` ts
// other importsimport  from '@/lib/types';
/* ...rest of the file... */
export const explainQuery = async (input: string, sqlQuery: string) => 
      Generated SQL Query:      $`,      schema: explanationSchema,      output: 'array',    });    return result.object;  } catch (e) };
```
...
``` ts
/* ...other imports... */import  from '@/app/actions';
...
``` ts
/* ...rest of the file... */
export const configSchema = z  .object()  .describe('Chart configuration object');
...
``` ts
/* ...other imports... */import  from '@/lib/types';
/* ...rest of the file... */
export const generateChartConfig = async (  results: Result[],  userQuery: string,) =>  = await generateObject(,        legend: true      }
      User Query:      $
      Data:      $`,      schema: configSchema,    });
    // Override with shadcn theme colors    const colors: Record<string, string> = ;    config.yKeys.forEach((key, index) => ))`;    });
    const updatedConfig = ;    return ;  } catch (e) };
```
...
``` typescript
/* ...other imports... */import  from './actions';
/* ...rest of the file... */const handleSubmit = async (suggestion?: string) => 
  setLoading(true);  setLoadingStep(1);  setActiveQuery('');
  try 
    setActiveQuery(query);    setLoadingStep(2);
    const companies = await runGeneratedSQLQuery(query);    const columns = companies.length > 0 ? Object.keys(companies[0]) : [];    setResults(companies);    setColumns(columns);
    setLoading(false);
    const  = await generateChartConfig(companies, question);    setChartConfig(config);  } catch (e) };
/* ...rest of the file... */
```
