...
``` ts
import  from 'anthropic-vertex-ai';
```
...
- **baseURL** *string*
  ...
  Use a different URL prefix for API calls, e.g., to use proxy servers. The default prefix is `https://-aiplatform.googleapis.com/v1`.
...
``` ts
import  from 'anthropic-vertex-ai';import  from 'ai';
