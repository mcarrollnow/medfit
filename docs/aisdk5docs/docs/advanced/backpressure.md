...
``` jsx
// A generator that will yield positive integersasync function* integers() `);    yield i++;
    await sleep(100);  }}function sleep(ms) 
// Wraps a generator into a ReadableStreamfunction createStream(iterator)       controller.close();    },  });}
// Collect data from streamasync function run()  = await reader.read();    console.log(`read $`);
    await sleep(1_000);  }}run();
```


In this example, we create an async-generator that yields positive integers, a `ReadableStream` that wraps our integer generator, and a reader which will read values out of our stream. Notice, too, that our integer generator logs out `"yielding $"`, and our reader logs out `"read $"`. Both take an arbitrary amount of time to process data, represented with a 100ms sleep in our generator, and a 1sec sleep in our reader.
...
// A generator that will yield positive integersasync function* integers() `);    yield i++;
    await sleep(100);  }}function sleep(ms) 
// Wraps a generator into a ReadableStreamfunction createStream(iterator)       controller.close();    },  });}// Collect data from streamasync function run()  = await reader.read();    console.log(`read $`);
...
``` jsx
// Wraps a generator into a ReadableStreamfunction createStream(iterator)  = await iterator.next();
      if (done)  else     },  });}
```
