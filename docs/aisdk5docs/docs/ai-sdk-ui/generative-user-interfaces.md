...
export default function Page()  = useChat();
  ...
  return (    <div>      >          <div></div>          <div>            ></span>;              }              return null;            })}          </div>        </div>      ))}
...
export const weatherTool = createTool(),  execute: async function () ;  },});
export const tools = ;
```
...
``` ts
import  from '@ai-sdk/openai';import  from 'ai';import  from '@/ai/tools';
export async function POST(request: Request) :  = await request.json();
  const result = streamText();
  return result.toUIMessageStreamResponse();}
```
...
``` tsx
type WeatherProps = ;
export const Weather = (: WeatherProps) => </h2>      <p>Condition: </p>      <p>Temperature: °C</p>    </div>  );};
```
...
To check if the model has called a tool, you can check the `parts` array of the UIMessage object for tool-specific parts. In AI SDK 5.0, tool parts use typed naming: `tool-$` instead of generic types.
...
``` tsx
'use client';
import  from '@ai-sdk/react';import  from 'react';import  from '@/components/weather';
export default function Page()  = useChat();
  const handleSubmit = (e: React.FormEvent) => );    setInput('');  };
  return (    <div>      >          <div></div>          <div>            ></span>;              }
              if (part.type === 'tool-displayWeather') >Loading weather...</div>;                  case 'output-available':                    return (                      <div key=>                        <Weather  />                      </div>                    );                  case 'output-error':                    return <div key=>Error: </div>;                  default:                    return null;                }              }
              return null;            })}          </div>        </div>      ))}
      <form onSubmit=>        <input          value=          onChange=          placeholder="Type a message..."        />        <button type="submit">Send</button>      </form>    </div>  );}
```
...
``` ts
// Add a new stock toolexport const stockTool = createTool(),  execute: async function () ;  },});
// Update the tools objectexport const tools = ;
```
...
``` tsx
type StockProps = ;
export const Stock = (: StockProps) => </p>      <p>Price: $</p>    </div>  );};
```
...
``` tsx
'use client';
import  from '@ai-sdk/react';import  from 'react';import  from '@/components/weather';import  from '@/components/stock';
export default function Page()  = useChat();
  const handleSubmit = (e: React.FormEvent) => );    setInput('');  };
  return (    <div>      >          <div></div>          <div>            ></span>;              }
              if (part.type === 'tool-displayWeather') >Loading weather...</div>;                  case 'output-available':                    return (                      <div key=>                        <Weather  />                      </div>                    );                  case 'output-error':                    return <div key=>Error: </div>;                  default:                    return null;                }              }
              if (part.type === 'tool-getStockPrice') >Loading stock price...</div>;                  case 'output-available':                    return (                      <div key=>                        <Stock  />                      </div>                    );                  case 'output-error':                    return <div key=>Error: </div>;                  default:                    return null;                }              }
              return null;            })}          </div>        </div>      ))}
