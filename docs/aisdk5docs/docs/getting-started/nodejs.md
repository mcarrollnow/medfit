...
``` ts
import  from '@ai-sdk/openai';import  from 'ai';import 'dotenv/config';import * as readline from 'node:readline/promises';
const terminal = readline.createInterface();
const messages: ModelMessage[] = [];
async function main() );
    const result = streamText();
    let fullResponse = '';    process.stdout.write('\nAssistant: ');    for await (const delta of result.textStream)     process.stdout.write('\n\n');
    messages.push();  }}
...
- Call [`streamText`](../reference/ai-sdk-core/stream-text.html), which is imported from the `ai` package. This function accepts a configuration object that contains a `model` provider and `messages`.
...
``` ts
import  from '@ai-sdk/openai';import  from 'ai';import 'dotenv/config';import  from 'zod';import * as readline from 'node:readline/promises';
const terminal = readline.createInterface();
const messages: ModelMessage[] = [];
async function main() );
...
async function main() );
    const result = streamText(),          execute: async () => ;          },        }),      },    });
    let fullResponse = '';    process.stdout.write('\nAssistant: ');    for await (const delta of result.textStream)     process.stdout.write('\n\n');
    console.log(await result.toolCalls);    console.log(await result.toolResults);    messages.push();  }}
main().catch(console.error);
```
...
const messages: ModelMessage[] = [];
async function main() );
    const result = streamText(),          execute: async () => ;          },        }),      },      stopWhen: stepCountIs(5),      onStepFinish: async () =>       },    });
    let fullResponse = '';    process.stdout.write('\nAssistant: ');    for await (const delta of result.textStream)     process.stdout.write('\n\n');
    messages.push();  }}
main().catch(console.error);
```
...
const messages: ModelMessage[] = [];
async function main() );
    const result = streamText(),          execute: async () => ;          },        }),        convertFahrenheitToCelsius: tool(),          execute: async () => ;          },        }),      },      stopWhen: stepCountIs(5),      onStepFinish: async () =>       },    });
    let fullResponse = '';    process.stdout.write('\nAssistant: ');    for await (const delta of result.textStream)     process.stdout.write('\n\n');
    messages.push();  }}
main().catch(console.error);
```
