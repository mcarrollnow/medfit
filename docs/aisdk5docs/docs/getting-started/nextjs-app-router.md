...
Be sure to select yes when prompted to use the App Router and Tailwind CSS. If you are looking for the Next.js Pages Router quickstart guide, you can find it [here](nextjs-pages-router.html).
...
Install `ai`, `@ai-sdk/react`, and `@ai-sdk/openai`, the AI package, AI SDK's React hooks, and AI SDK's [OpenAI provider](../../providers/ai-sdk-providers/openai.html) respectively.
...
Now that you have a Route Handler that can query an LLM, it's time to setup your frontend. The AI SDK's [UI](../ai-sdk-ui.html) package abstracts the complexity of a chat interface into one hook, [`useChat`](../reference/ai-sdk-ui/use-chat.html).
...
'use client';
import  from '@ai-sdk/react';import  from 'react';
export default function Chat()  = useChat();  return (    <div className="flex flex-col w-full max-w-md py-24 mx-auto stretch">       className="whitespace-pre-wrap">                    -$`}></div>;              case 'tool-weather':                return (                  <pre key=-$`}>                                      </pre>                );            }          })}        </div>      ))}
      <form        onSubmit=);          setInput('');        }}      >        <input          className="fixed dark:bg-zinc-900 bottom-0 w-full max-w-md p-2 mb-8 border border-zinc-300 dark:border-zinc-800 rounded shadow-xl"          value=          placeholder="Say something..."          onChange=        />      </form>    </div>  );}
```
...
'use client';
import  from '@ai-sdk/react';import  from 'react';
export default function Chat()  = useChat();  return (    <div className="flex flex-col w-full max-w-md py-24 mx-auto stretch">       className="whitespace-pre-wrap">                    -$`}></div>;              case 'tool-weather':              case 'tool-convertFahrenheitToCelsius':                return (                  <pre key=-$`}>                                      </pre>                );            }          })}        </div>      ))}
      <form        onSubmit=);          setInput('');        }}      >        <input          className="fixed dark:bg-zinc-900 bottom-0 w-full max-w-md p-2 mb-8 border border-zinc-300 dark:border-zinc-800 rounded shadow-xl"          value=          placeholder="Say something..."          onChange=        />      </form>    </div>  );}
```
