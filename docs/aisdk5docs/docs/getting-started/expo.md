...
``` tsx
import  from '@/utils';import  from '@ai-sdk/react';import  from 'ai';import  from 'expo/fetch';import  from 'react';import  from 'react-native';
export default function App()  = useChat(),    onError: error => console.error(error, 'ERROR'),  });
...
``` ts
import  from 'react-native';import structuredClone from '@ungap/structured-clone';
if (Platform.OS !== 'web')  = await import(      'react-native/Libraries/Utilities/PolyfillFunctions'    );
    const  = await import(      '@stardazed/streams-text-encoding'    );
