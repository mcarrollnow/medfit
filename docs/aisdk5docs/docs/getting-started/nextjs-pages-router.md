...
Be sure to select no when prompted to use the App Router. If you are looking for the Next.js App Router quickstart guide, you can find it [here](nextjs-app-router.html).
...
Now that you have an API route that can query an LLM, it's time to setup your frontend. The AI SDK's [UI](../ai-sdk-ui.html) package abstract the complexity of a chat interface into one hook, [`useChat`](../reference/ai-sdk-ui/use-chat.html).
