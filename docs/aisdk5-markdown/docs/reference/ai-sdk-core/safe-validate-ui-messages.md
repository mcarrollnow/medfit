AI SDK Core: safeValidateUIMessages
...
[AI SDK Core](../../ai-sdk-core.html)safeValidateUIMessages

# [`safeValidateUIMessages`](#safevalidateuimessages)

`safeValidateUIMessages` is an async function that validates UI messages like [`validateUIMessages`](validate-ui-messages.html), but instead of throwing it returns an object with a `success` key and either `data` or `error`.
