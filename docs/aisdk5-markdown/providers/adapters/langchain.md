...
[Adapters](../adapters.html)LangChain
...
The [`@ai-sdk/langchain` package](../../docs/reference/stream-helpers/langchain-adapter.html) uses the result from [LangChain ExpressionLanguage streaming](https://js.langchain.com/docs/how_to/streaming) to pipe text to the client. `toDataStreamResponse()` is compatible with the LangChain Expression Language `.stream()` function response.
