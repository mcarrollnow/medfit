...
Work with your support team to [define clear success criteria](../../test-and-evaluate/define-success.html) and write [detailed evaluations](../../test-and-evaluate/develop-tests.html) with measurable benchmarks and goals. Here are criteria and benchmarks that can be used to evaluate how successfully Claude performs the defined tasks:
...
It’s hard to know how well your prompt works without deploying it in a test production setting and [running evaluations](../../test-and-evaluate/develop-tests.html) so let’s build a small application using our prompt, the Anthropic SDK, and streamlit for a user interface. In a file called `chatbot.py`, start by setting up the ChatBot class, which will encapsulate the interactions with the Anthropic SDK. The class should have two main methods: `generate_message` and `process_user_input`.
...
Prompting often requires testing and optimization for it to be production ready. To determine the readiness of your solution, evaluate the chatbot performance using a systematic process combining quantitative and qualitative methods. Creating a [strong empirical evaluation](../../test-and-evaluate/develop-tests.html#building-evals-and-test-cases) based on your defined success criteria will allow you to optimize your prompts.
...
-   [Reduce hallucination](../../test-and-evaluate/strengthen-guardrails/reduce-hallucinations.html): Implement fact-checking mechanisms and [citations](https://github.com/anthropics/anthropic-cookbook/blob/main/misc/using_citations.ipynb) to ground responses in provided information.
...
-   [Mitigate jailbreaks](../../test-and-evaluate/strengthen-guardrails/mitigate-jailbreaks.html): Use methods like harmlessness screens and input validation to prevent users from exploiting model vulnerabilities, aiming to generate inappropriate content.
...
-   [Keep Claude in character](../../test-and-evaluate/strengthen-guardrails/keep-claude-in-character.html): Prevent Claude from changing their style of context, even during long, complex interactions.
...
1.  Use the [Anthropic Streaming API](../../build-with-claude/streaming.html) to support streaming responses.
...
-   Ensure that you are making the most out of your prompts and optimizing through prompt engineering. Use our [prompt engineering guides](../../build-with-claude/prompt-engineering/overview.html) to write the most effective prompts.
-   Add additional [tools](../../agents-and-tools/tool-use/overview.html) to the prompt (which can include [prompt chains](../../build-with-claude/prompt-engineering/chain-prompts.html)) and see if you can achieve the functionality required.
...
[Ticket routing](ticket-routing.html)[Content moderation](content-moderation.html)
