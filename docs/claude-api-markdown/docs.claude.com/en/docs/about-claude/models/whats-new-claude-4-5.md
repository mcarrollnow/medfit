...
-   [Token count optimizations](#token-count-optimizations)
...
Claude Sonnet 4.5 performs significantly better on coding tasks when [extended thinking](../../build-with-claude/extended-thinking.html) is enabled. Extended thinking is disabled by default, but we recommend enabling it for complex coding work. Be aware that extended thinking impacts [prompt caching efficiency](../../build-with-claude/prompt-caching.html#caching-with-thinking-blocks). See the [migration guide](migrating-to-claude-4.html#extended-thinking-recommendations) for configuration details.
...
Extended thinking must be enabled explicitly by adding a `thinking` parameter to your API requests. See the [Extended thinking documentation](../../build-with-claude/extended-thinking.html) for implementation details.

Claude Haiku 4.5 performs significantly better on coding and reasoning tasks when [extended thinking](../../build-with-claude/extended-thinking.html) is enabled. Extended thinking is disabled by default, but we recommend enabling it for complex problem-solving, coding work, and multi-step reasoning. Be aware that extended thinking impacts [prompt caching efficiency](../../build-with-claude/prompt-caching.html#caching-with-thinking-blocks). See the [migration guide](migrating-to-claude-4.html#extended-thinking-recommendations) for configuration details.
...
This is the first Haiku model with native context awareness capabilities. For prompting guidance, see [Claude 4 best practices](../../build-with-claude/prompt-engineering/claude-4-best-practices.html#context-awareness-and-multi-window-workflows).
...
The new [memory tool](../../agents-and-tools/tool-use/memory-tool.html) enables Claude to store and retrieve information outside the context window:
...
Available in Claude Sonnet 4, Sonnet 4.5, Haiku 4.5, Opus 4, and Opus 4.1. Requires [beta header](../../../api/beta-headers.html): `context-management-2025-06-27`
...
Use [context editing](../../build-with-claude/context-editing.html) for intelligent context management through automatic tool call clearing:
...
When using Claude 4 models, you should update your application to [handle `refusal` stop reasons](../../test-and-evaluate/strengthen-guardrails/handle-streaming-refusals.html).
...
For more information, see the [Extended thinking documentation](../../build-with-claude/extended-thinking.html#summarized-thinking).
...
Interleaved thinking is in beta. To enable interleaved thinking, add [the beta header](../../../api/beta-headers.html) `interleaved-thinking-2025-05-14` to your API request.

For more information, see the [Extended thinking documentation](../../build-with-claude/extended-thinking.html#interleaved-thinking).
...
For comprehensive guidance on working with these models, see [Claude 4 prompt engineering best practices](../../build-with-claude/prompt-engineering/claude-4-best-practices.html).
...
For more information, see the [Text editor tool documentation](../../agents-and-tools/tool-use/text-editor-tool.html).
...
For more details, see the [pricing documentation](../pricing.html).
...
Breaking changes and migration requirements vary depending on which model you’re upgrading from. For detailed migration instructions, including step-by-step guides, breaking changes, and migration checklists, see [Migrating to Claude 4.5](migrating-to-claude-4.html). The migration guide covers the following scenarios:
...
[Choosing a model](choosing-a-model.html)[Migrating to Claude 4.5](migrating-to-claude-4.html)
