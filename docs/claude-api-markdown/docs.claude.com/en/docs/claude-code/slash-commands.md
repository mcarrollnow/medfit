...
| `/cost` | Show token usage statistics (see [cost tracking guide](costs.html#using-the-cost-command) for subscription-specific details) |
...
| `/permissions` | View or update [permissions](iam.html#configuring-permissions) |
...
Include file contents in commands using the `@` prefix to [reference files](common-workflows.html#reference-files-and-directories). For example:
...
Slash commands can trigger extended thinking by including [extended thinking keywords](common-workflows.html#use-extended-thinking).
...
| `model` | Specific model string (see [Models overview](../about-claude/models/overview.html)) | Inherits from the conversation |
...
[Plugins](plugins.html) can provide custom slash commands that integrate seamlessly with Claude Code. Plugin commands work exactly like user-defined commands but are distributed through [plugin marketplaces](plugin-marketplaces.html).
...
When configuring [permissions for MCP tools](iam.html#tool-specific-permission-rules), note that **wildcards are not supported**:
...
Both slash commands and Skills can coexist. Use the approach that fits your needs. Learn more about [Agent Skills](skills.html).
...
-   [Plugins](plugins.html) - Extend Claude Code with custom commands through plugins
-   [Identity and Access Management](iam.html) - Complete guide to permissions, including MCP tool permissions
...
-   [Memory management](memory.html) - Managing Claude’s memory across sessions

Was this page helpful?

YesNo

[Interactive mode](interactive-mode.html)[Checkpointing](checkpointing.html)
