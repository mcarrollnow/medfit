...
[Plugins](plugins.html) can bundle MCP servers, automatically providing tools and integrations when the plugin is enabled. Plugin MCP servers work identically to user-configured servers. **How plugin MCP servers work**:
...
See the [plugin components reference](plugins-reference.html#mcp-servers) for details on bundling MCP servers with plugins.
...
[GitLab CI/CD](gitlab-ci-cd.html)[Troubleshooting](troubleshooting.html)
