...
For a quickstart guide with examples, see [Get started with Claude Code hooks](hooks-guide.html).
...
Claude Code hooks are configured in your [settings files](settings.html):
...
[Plugins](plugins.html) can provide hooks that integrate seamlessly with your user and project hooks. Plugin hooks are automatically merged with your configuration when plugins are enabled. **How plugin hooks work**:
...
See the [plugin components reference](plugins-reference.html#hooks) for details on creating plugin hooks.
...
-   `Task` - Subagent tasks (see [subagents documentation](sub-agents.html))
-   `Bash` - Shell commands
...
Claude Code hooks work seamlessly with [Model Context Protocol (MCP) tools](mcp.html). When MCP servers provide tools, they appear with a special naming pattern that you can match in your hooks.
...
For practical examples including code formatting, notifications, and file protection, see [More Examples](hooks-guide.html#more-examples) in the get started guide.
...
[Checkpointing](checkpointing.html)[Plugins reference](plugins-reference.html)
