...
These changes apply to the [local project level](settings.html) and are saved in `.claude/settings.local.json`.
...
Output Styles vs. [Agents](sub-agents.html)
...
Output Styles vs. [Custom Slash Commands](slash-commands.html)
...
[Agent Skills](skills.html)[Hooks](hooks-guide.html)
