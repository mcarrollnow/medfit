...
For more information about the response format of extended thinking, see the [Messages API Reference](../../api/messages.html).
...
Claude Sonnet 3.7 continues to return full thinking output.In rare cases where you need access to full thinking output for Claude 4 models, [contact our sales team](../../../cdn-cgi/l/email-protection.html#b4c7d5d8d1c7f4d5dac0dcc6dbc4ddd79ad7dbd9).
...
Extended thinking can be used alongside [tool use](../agents-and-tools/tool-use/overview.html), allowing Claude to reason through tool selection and results processing. When using extended thinking with tool use, be aware of the following limitations:
...
2.  **Context maintenance**: While tool results appear as user messages in the API structure, they’re part of a continuous reasoning flow. Preserving thinking blocks maintains this conceptual flow across multiple API calls. For more information on context management, see our [guide on context windows](context-windows.html).
...
To enable interleaved thinking, add [the beta header](../../api/beta-headers.html) `interleaved-thinking-2025-05-14` to your API request. Here are some important considerations for interleaved thinking:

-   With interleaved thinking, the `budget_tokens` can exceed the `max_tokens` parameter, as it represents the total budget across all thinking blocks within one assistant turn.
-   Interleaved thinking is only supported for [tools used via the Messages API](../agents-and-tools/tool-use/overview.html).
...
-   On 3rd-party platforms (e.g., [Amazon Bedrock](../../api/claude-on-amazon-bedrock.html) and [Vertex AI](../../api/claude-on-vertex-ai.html)), if you pass `interleaved-thinking-2025-05-14` to any model aside from Claude Opus 4.1, Opus 4, or Sonnet 4, your request will fail.
...
[Prompt caching](prompt-caching.html) with thinking has several important considerations:

Extended thinking tasks often take longer than 5 minutes to complete. Consider using the [1-hour cache duration](prompt-caching.html#1-hour-cache-duration) to maintain cache hits across longer thinking sessions and multi-step workflows.
...
You can read through our [guide on context windows](context-windows.html) for a more thorough deep dive.
...
The diagram below demonstrates the specialized token management when extended thinking is enabled: ![Context window diagram with extended thinking](../../../../mintcdn.com/anthropic-claude-docs/LF5WV0SNF6oudpT5/images/context-window-thinking%EF%B9%96fit=max&auto=format&n=LF5WV0SNF6oudpT5&q=85&s=3ad289c01610a87c8ec1214faa09578d.svg) The effective context window is calculated as:
...
We recommend using the [token counting API](token-counting.html) to get accurate token counts for your specific use case, especially when working with multi-turn conversations that include thinking.
...
The diagram below illustrates token management for extended thinking with tool use: ![Context window diagram with extended thinking and tool use](../../../../mintcdn.com/anthropic-claude-docs/LF5WV0SNF6oudpT5/images/context-window-thinking-tools%EF%B9%96fit=max&auto=format&n=LF5WV0SNF6oudpT5&q=85&s=557310c0bf57d88b7a6e550abd35bc75.svg)
...
-   Potentially use the [token counting endpoints](token-counting.html) more frequently
...
-   `signature` values are compatible across platforms (Claude APIs, [Amazon Bedrock](../../api/claude-on-amazon-bedrock.html), and [Vertex AI](../../api/claude-on-vertex-ai.html)). Values generated on one platform will be compatible with another.
...
-   **Large budgets:** For thinking budgets above 32k, we recommend using [batch processing](batch-processing.html) to avoid networking issues. Requests pushing the model to think above 32k tokens causes long running requests that might run up against system timeouts and open connection limits.
...
-   Thinking isn’t compatible with `temperature` or `top_k` modifications as well as [forced tool use](../agents-and-tools/tool-use/implement-tool-use.html#forcing-tool-use).
...
-   **Prompt engineering:** Review our [extended thinking prompting tips](prompt-engineering/extended-thinking-tips.html) if you want to maximize Claude’s thinking capabilities.
...
[Context editing](context-editing.html)[Streaming Messages](streaming.html)
