...
import { useSupportUIData } from "@/hooks/useSupportUIData"
...
export function MobileSidebar({ isOpen, onClose }: MobileSidebarProps) {
  const { tickets: realTickets, loading, fetchCustomer, fetchMessages, refetch } = useSupportUIData()
  ...
  const handleAssignToAI = async (ticketId: string) => {
    try {
      const response = await fetch('/api/support-ui/tickets', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ticketId, ai_handling: true })
      })
      if (response.ok) {
        await refetch()
      }
  ...
  // Load customer data and messages when ticket is expanded
  useEffect(() => {
    if (expandedTicketId && realTickets.length > 0) {
      const ticket = realTickets.find((t: any) => t.id === expandedTicketId)
      if (ticket?.customer?.id) {
        fetchCustomer(ticket.customer.id).then(setCustomerData)
        fetchMessages(expandedTicketId).then(setTicketMessages)
      }
    }
  }, [expandedTicketId, realTickets])

  // Transform real tickets into display format
  const tickets = realTickets.map((ticket: any) => {
    const customerName = ticket.customer?.user?.first_name 
      ? `${ticket.customer.user.first_name} ${ticket.customer.user.last_name || ''}`.trim()
      : ticket.customer?.company_name || 'Unknown'
    ...
    lastMessage: ticket.message || 'No messages',
    time: new Date(ticket.created_at).toLocaleTimeString(),
    priority: ticket.priority || 'medium',
    description: ticket.message || '',
    orderDetails: {
      orderNumber: ticket.order?.order_number || '',
      orderDate: ticket.order?.created_at ? new Date(ticket.order.created_at).toLocaleDateString() : '',
      total: ticket.order?.total_amount ? `$${ticket.order.total_amount}` : '$0',
      items: customerData?.orders?.[0]?.order_items?.map((item: any) => ({
        name: item.product?.name || item.product_name || '',
        quantity: item.quantity || 0,
        price: `$${item.unit_price || 0}`
      })) || [],
      subtotal: ticket.order?.total_amount ? `$${ticket.order.total_amount}` : '$0',
      shipping: '$0',
      tax: '$0',
      walletAddress: '',
      trackingNumber: ticket.order?.tracking_number || '',
      timeline: []
    },
    fullConversation: ticketMessages.map((msg: any) => ({
    ...
    adminOnlyConversation: ticketMessages.filter((msg: any) => msg.is_private).map((msg: any) => ({
      date: new Date(msg.created_at).toLocaleDateString(),
      messages: [{
        sender: msg.sender_type,
        senderName: msg.sender_name || '',
    ...
    customerProfile: {
      ...
      totalSpent: customerData?.orders?.reduce((sum: number, order: any) => sum + parseFloat(order.total_amount || 0), 0) || 0,
      totalOrders: customerData?.orders?.length || 0,
      averageOrderValue: customerData?.orders?.length ? 
        (customerData.orders.reduce((sum: number, order: any) => sum + parseFloat(order.total_amount || 0), 0) / customerData.orders.length) : 0,
      customerSince: ticket.customer?.created_at || '',
      orderHistory: customerData?.orders?.map((order: any) => ({
  ...
  // Transform tickets into conversations for history tab
  const conversations = tickets.map((ticket: any) => ({
    id: ticket.id,
    title: ticket.title,
    customer: ticket.customer,
    preview: ticket.lastMessage,
    time: ticket.time,
    messageCount: ticketMessages.length || 0,
  }))

  // Keep mock conversations for reference
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                              ...
                              <Button 
                                ...
                                variant="outline" 
                                className="w-full gap-2 bg-transparent"
                                onClick={() => handleAssignToAI(ticket.id)}
                              >
          ...
          {activeTab === "history" && activeToolView === "list" && (
            <div className="p-4 space-y-3">
              ...
              {filteredConversations.map((conversation) => {
                // Find the actual ticket data
                const ticket = tickets.find(t => t.id === conversation.id);
                ...
                return (
                  <Card key={conversation.id} className="overflow-hidden">
                    ...
                    <div
                      className="p-4 cursor-pointer hover:bg-accent transition-colors"
                      onClick={() => {
                        setExpandedTicketId(expandedTicketId === conversation.id ? null : conversation.id)
                      }}
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <ChevronDown
                          className={cn(
                            "w-4 h-4 text-muted-foreground transition-transform",
                            expandedTicketId === conversation.id && "rotate-180",
                          )}
                        />
                      </div>
                    </div>

                    {/* Expanded View - Show full ticket details */}
                    {expandedTicketId === conversation.id && ticket && (
                      <div className="border-t border-border bg-muted/30">
                        <div className="flex border-b border-border bg-card">
                          {getRotatingTabs(activeTicketTab).map((tab) => (
                            <button
                        ...
                        <div className="p-4">
                          ...
                          {activeTicketTab === "profile" && ticket.customerProfile && (
                            <div className="space-y-4">
                              <h5 className="text-sm font-semibold text-foreground">Customer Profile</h5>
                              {/* Profile content */}
                            </div>
                          )}
                        </div>
