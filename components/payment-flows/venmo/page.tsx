...
function VenmoGuideContent({ orderTotal, merchantWallet, orderId, onPaymentComplete }: {
  orderTotal: number
  ...
  const iosDownloadLink = "https://apps.apple.com/us/app/venmo/id351727428"
  const androidDownloadLink = "https://play.google.com/store/apps/details?id=com.venmo"
  const venmoDeepLink = "venmo://"
  ...
  const prevStep = () => {
    if (currentStep > 1) {
      if (currentStep === 4 && walletConfirmed) {
  ...
  return (
    <>
      <div className="mb-12 text-center">
          ...
          <h1 className="mb-3 text-4xl font-bold tracking-tight sm:text-5xl">Using Venmo Crypto</h1>
          <p className="text-lg text-muted-foreground">Follow these simple steps to pay with Ethereum on Venmo</p>
        </div>
        ...
        {showCart ? (
                ...
                <div className="rounded-md border border-border bg-muted p-6">
                  ...
                  <div className="flex justify-center mb-4">
                    <div className="rounded-md bg-white p-4 inline-block">
                      <QRCode value={merchantWallet} size={180} />
                    </div>
                  </div>
                  <div className="text-center space-y-3">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Scan QR code in Venmo, enter amount, press send, then return here to complete order.
                    </p>
                  </div>
                </div>
                <button 
                ...
                {!addressPasted ? (
                    ...
                    {!addressCopied ? (
                      <>
                        <p className="text-sm text-muted-foreground text-center">
                          Tap to copy wallet address, then open Venmo to paste in Send
                        </p>
                        <button
                          onClick={copyAddress}
                          className="flex w-full items-center justify-center gap-2 rounded-md bg-foreground px-6 py-4 text-sm font-medium text-background transition-colors hover:bg-foreground/90"
                  ...
                  <>
                    {!paymentAmountCopied ? (
                      <>
                        <p className="text-sm text-muted-foreground text-center">
                          Tap amount to copy, then open Venmo to paste and send
                        </p>
                        <button
          ...
          <div className="space-y-6">
            {currentStep >= 1 && (
              ...
              >
                <h2 className="mb-3 text-2xl font-semibold">Step 1: Download Venmo</h2>
                <p className="mb-6 text-muted-foreground">Download the Venmo app or skip if already installed</p>
                {currentStep === 1 && (
              ...
              >
                ...
                <p className="mb-6 text-muted-foreground">
                  <span className="lg:hidden">
                    Open Venmo, enable crypto, complete verification. Return here after.
                  </span>
                  <span className="hidden lg:inline">
                    Open Venmo, tap Crypto tab, accept terms, complete verification
                  </span>
                </p>
                {currentStep === 2 && (
                  <div className="space-y-3">
                ...
                {currentStep === 3 && !purchaseComplete ? (
                  <>
                    ...
                    <p className="mb-6 text-muted-foreground">
                      <span className="lg:hidden">
                        {!amountCopied
                          ? "Tap amount to copy, then open Venmo to buy ETH. Return here after purchase"
                          : "Amount copied! Tap to open Venmo, buy ETH, then return here"}
                      </span>
                      <span className="hidden lg:inline">Enter this exact amount in Venmo</span>
                    </p>
                    {!amountCopied ? (
                      <button
                        onClick={copyAmount}
            ...
            {currentStep >= 4 && !walletConfirmed && (
              <div className="rounded-lg border border-foreground/20 bg-card p-8 transition-all duration-500">
                <h2 className="mb-3 text-2xl font-semibold">Step 4: Send to Wallet</h2>
                <p className="mb-6 text-muted-foreground">
                  {!walletConfirmed ? (
                    <>
                      <span className="hidden lg:inline">Scan QR code from Venmo crypto section, tap Send</span>
                      <span className="lg:hidden">Copy address, open Venmo crypto, tap Send, paste address</span>
                    </>
                  ) : (
                    "Enter this amount to complete the transfer"
                  )}
                </p>
                ...
                {!walletConfirmed ? (
                  <>
                    <div className="mb-6 rounded-md border border-border bg-muted p-5">
                      <div className="mb-1 text-xs font-medium text-muted-foreground">Wallet Address</div>
                      <div className="break-all font-mono text-sm mb-4">{merchantWallet}</div>
                  ...
                  <>
                    <div className="mb-6 rounded-md border-2 border-foreground/20 bg-muted p-8 text-center">
                      <div className="text-5xl font-bold">{orderAmount}</div>
                    </div>
                    <div className="flex gap-3">
                      <button
            ...
            {currentStep > totalSteps && (
              <div className="mt-8 rounded-lg border border-border bg-card p-8 text-center">
                ...
                <p className="text-muted-foreground">
                  Your Ethereum transfer is complete. The transaction may take a few minutes to process.
                </p>
              </div>
            )}
          </div>
...
export function VenmoPaymentFlow(props: {
  orderTotal: number
  merchantWallet: string
  orderId: string
  onPaymentComplete: () => void
  ...
  return <VenmoGuideContent {...props} />
}

export default function VenmoGuide() {
  return (
    <PaymentFlowWrapper>
      {(props) => <VenmoGuideContent {...props} />}
    </PaymentFlowWrapper>
  )
}
