        ...
        <div className="relative z-10">
          <CollapsibleTrigger className="w-full p-6 text-left focus:outline-none">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-full bg-foreground/5 flex items-center justify-center border border-border">
              ...
              <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto">
                ...
                <div
                  className={cn(
                    "h-8 w-8 rounded-full bg-foreground/5 flex items-center justify-center transition-transform duration-300 border border-border",
                    isOpen && "rotate-180 bg-foreground/10",
                  )}
                >
          ...
          <CollapsibleContent>
            <div className="px-6 pb-6 pt-0">
              <div className="mt-4 border-t border-border pt-6 space-y-8">
                {/* Order Details Section with Share Button */}
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <h4 className="text-sm font-medium text-foreground/60 flex items-center gap-2 uppercase tracking-wider">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-8 gap-2 text-foreground/60 hover:text-foreground hover:bg-foreground/10"
                        >
                          <Share2 className="h-4 w-4" />
                          Share Details
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-48 bg-background/90 border-border backdrop-blur-xl">
                        <DropdownMenuItem
                          className="text-foreground/80 focus:text-foreground focus:bg-foreground/10 cursor-pointer gap-2"
                  ...
                  <div className="space-y-4">
                    {order.items.length === 0 ? (
                      ...
                      order.items.map((item) => (
                        <div
                          key={item.id}
                          className="flex flex-col sm:flex-row sm:items-center justify-between text-sm p-4 rounded-2xl bg-foreground/5 border border-border transition-colors hover:bg-foreground/10 gap-3"
                        >
                          <div className="flex items-center gap-4">
                            <span className="text-muted-foreground font-mono text-xs bg-foreground/5 px-2 py-1 rounded-md">
                              x{item.quantity}
                            </span>
                            <span className="text-foreground font-medium text-base">{item.productName}</span>
                          </div>
                          <div className="flex items-center gap-6">
                ...
                {order.tracking && order.tracking.timeline.length > 0 && (
                  <div>
                    <h4 className="text-sm font-medium text-foreground/60 mb-6 flex items-center gap-2 uppercase tracking-wider">
                      <Truck className="h-4 w-4" /> Shipment Tracking
                    </h4>
                    <div className="relative pl-4 border-l border-border ml-2 space-y-8">
                      {order.tracking.timeline.map((event, index) => (
                        <div key={index} className="relative">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-1 sm:gap-4">
                            <div>
                              <p className={cn("text-sm font-medium", index === 0 ? "text-foreground" : "text-foreground/60")}>
                                {event.status}
                              </p>
                    ...
                    <div className="mt-6 flex flex-wrap items-center gap-4 text-xs text-muted-foreground bg-foreground/5 p-3 rounded-lg border border-border">
                      <span>Carrier: {order.tracking.carrier}</span>
                      <span className="w-px h-3 bg-foreground/10 hidden sm:block" />
                      <span className="font-mono">Tracking: {order.tracking.trackingNumber}</span>
                    </div>
                  </div>
                )}

                <div className="flex justify-end items-center gap-6 text-sm border-t border-border pt-6">
                  <span className="text-muted-foreground uppercase tracking-wider">Order Total</span>
                  <span className="text-foreground font-light text-2xl">${order.total.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </CollapsibleContent>
